...
### POST /user/cart/:userId (Add to cart)
...
### PUT /user/cart/:userId (Remove product / Reduce a product's quantity from the cart)

- Updates a cart by either decrementing the quantity of a product by 1 or deleting a product from the cart.
- Get productId in request body.
- Get key 'updateProduct' in request body.
- Make sure that cart exist.
- Key 'updateProduct' denotes whether a product is to be decremented by 1({updateProduct: 0}) or its quantity has to be incremented by 1({updateProduct: 1}).
- Make sure the userId in params and in JWT token match.
- Make sure the user exist
- returns updatedCart in response body.
...
### DELETE /user/cart/:userId

- Deletes the cart for the user.
- Makes sure that cart exist.
- Makes sure the userId in params and in JWT token match.
- cart deleting means array of items is empty, totalItems is 0, totalPrice is 0.
- **Response format**
  - _**On success**_ - Returs HTTP 404 Status
  - _**On error**_ - Returns a suitable error message with a valid HTTP status code.
